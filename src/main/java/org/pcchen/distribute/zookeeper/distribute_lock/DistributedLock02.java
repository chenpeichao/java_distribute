...
/**
 * 分布式共享锁测试
 * 在指定节点(disLocks)下创建名为sub的EPHEMERAL_SEQUENTIAL节点；
 * 获取指定节点(disLocks)下所有子节点，排序，如果自己的编号最小，则获取锁
 * 否则watch排在自己前面的节点，坚挺到其删除后，进入第二步(重新检测排序是放置监听的节点发生连接失败，导致的节点删除情况)
 * 删除自身sub节点，释放连接
 *
 * @author ceek
 * @create 2020-07-21 10:53
 **/
public class DistributedLock02 implements Watcher {
    ...
    private static final String CONNECTION_STRING = "10.10.32.61:2181";
    ...
    private static final Logger LOG = LoggerFactory.getLogger(DistributedLock02.class);

    public DistributedLock02(int id) {
        this.threadId = id;
    ...
    public static void main(String[] args) {
        for (int i = 0; i < THREAD_NUM; i++) {
            ...
            new Thread() {
                ...
                public void run() {
                    try {
                        DistributedLock02 dc = new DistributedLock02(threadId);
